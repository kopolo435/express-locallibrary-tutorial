...
exports.genre_delete_get = asyncHandler(async (req, res, next) => {
  const [genre, genreBooks] = await Promise.all([
    Genre.findById(req.params.id).exec(),
    Book.find({ genre: req.params.id }).sort({ title: 1 }),
  ]);
...
exports.genre_delete_post = asyncHandler(async (req, res, next) => {
  const [genre, genreBooks] = await Promise.all([
    Genre.findById(req.params.id).exec(),
    Book.find({ genre: req.params.id }).sort({ tile: 1 }).exec(),
  ]);

  if (genreBooks > 0) {
    //There are books bound to the genre
    res.render("genre_delete", { title: "Delete Genre", genre, genreBooks });
  } else {
    //The genre can be deleted
...
exports.genre_update_post = [
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      res.render("genre_author", { genre, errors: errors.array() });
      return;
    } else {
      // Check if Genre with same name already exists.
      const genreExists = await Genre.findOne({ name: req.body.name })
